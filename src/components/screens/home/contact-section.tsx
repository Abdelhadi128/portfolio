...
export default function ContactSection() {
  ...
  return (
    <div id='contact' className='w-full min-h-screen md:h-screen flex flex-col items-center py-20'>
            ...
            <div className='w-full flex flex-col sm:grid md:grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-9 p-1 sm:p-0'>
              {contacts.map((contact)=>(  
                contact.name.toLowerCase() == 'email' ? 
                  <a href={`mailto:${contact.link}`} className='w-full group flex flex-col items-center sm:min-h-full bg-[#141517] p-5 border border-[#0a824b] md:border-[#27292c] hover:border-[#0a824b] rounded-xl overflow-hidden'>
                      {contact.icon}
